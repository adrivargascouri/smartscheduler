"""
Main entry point for the SmartScheduler application.
Handles GUI setup, database bootstrapping, and main dashboard logic.
"""
...
tree = None  # Global treeview reference

def seed_employees():
    """
    Seeds the database with default employees if none exist.
    """
...
# Seed employees after creating tables
seed_employees()
...
def schedule_appointment(name_entry, employee_opts, employee_combo, date_picker, time_combo):
    """
    Schedules an appointment after validating input fields.
    """
    ...
    ok, msg = schedule_appointment_with_validation(
        client_name, employee_label, start_time, end_time
    )
    if not ok:
...
def refresh_tree():
    """
    Refreshes the appointment treeview with current data.
    """
...
def export_appoint_selected():
    """
    Exports selected appointments to an .ics calendar file.
    """
    ...
    for item_id in seleccionadas:
        ...
        cliente = cita[0]
        empleado = cita[1]
        inicio_str = cita[2]  # Format: 'YYYY-MM-DD HH:MM:SS'
        fin_str = cita[3]
        estado = cita[4]
        ...
        event.created = datetime.utcnow().replace(tzinfo=timezone.utc)
...
def mark_selected(new_status: str):
    """
    Marks the selected appointment with a new status.
    """
...
def cancel_by_client(name_entry):
    """
    Cancels all appointments for a given client name.
    """
    client_name = name_entry.get().strip()
    if not client_name:
    ...
    msg = (
        "No scheduled appointments were found."
        if affected == 0 else
...
def launch_dashboard():
    """
    Launches the main dashboard UI.
    """
    # HEADER (navbar)
    ...
    logo_img = None
    try:
        logo_img = PhotoImage(file="logo.png").subsample(8, 8)
    ...
    tree = ttk.Treeview(
        tab_view,
        columns=("Client", "Employee", "Start", "End", "Status"),
        show="headings"
    )
    for col, w in zip(
        ("Client", "Employee", "Start", "End", "Status"),
        (140, 140, 140, 140, 100)
    ):
    ...
    tb.Button(
        ...
        bootstyle="primary",
        command=export_appoint_selected
...
def show_welcome():
    """
    Shows the animated welcome screen before entering the dashboard.
    """
